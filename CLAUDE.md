...
### Shared-Terminal Identity Confirmation (Stage 4 — now PIN-based)
...
- **Originally password-based** (verified server-side against Supabase Auth), but that tied the gate to the *shared terminal's own session validity* — `getFreshAccessToken()` would intermittently fail on a long-lived shift session (background-tab refresh-timer throttling etc.), surfacing as constant "Session expired" errors on the exact flow meant to fix accountability. **Replaced with a 4-digit PIN** per staffer (2026-07-06) that has nothing to do with the terminal's own login state.
- **`staff_pins`** table: `user_id` (PK) → `pin_hash` (`"salt:scryptHash"`, hashed via `lib/pin.ts`'s `hashPin()`/`verifyPin()` — Node's built-in `crypto.scrypt`, no new dependency), `failed_attempts`, `locked_until`. **RLS enabled with NO policies at all** (same pattern as `invoice_counters`) — a 4-digit space is brute-forceable offline in well under a second, so the hash must never be reachable via the anon key even though `users` reads are org-wide. Only service-role routes touch it.
- **`lib/IdentityConfirm.tsx`** — `IdentityConfirmProvider` (mounted in `app/dashboard/layout.tsx`, alongside `ConfirmProvider`) + a promise-based `useIdentityConfirm()`. A call site does `const actor = await confirmIdentity({ action, entityId }); if (!actor) return` — **unchanged signature**, so none of the 5 call sites needed edits. The dialog shows a **staff-name dropdown + 4-digit PIN**; a falsy result (cancel, wrong PIN, or lockout) aborts the action.
- **`app/api/confirm-identity/route.ts`** — looks up `staff_pins` by `(user_id, org_id)` (both sent from the client — `org_id` from `localStorage`, same as every other query in this app) and verifies the PIN via `verifyPin()`. **Deliberately does not require the shared terminal's own bearer token** — that dependency was the root cause of the recurring errors. Mitigated by a **5-attempt/15-minute lockout** per staffer (makes the 10,000-combination PIN space impractical to brute-force) plus `org_id` being a non-enumerable UUID. On success it writes the same `audit_logs` row as before: `action = 'confirm'`, `entity_type = 'confirmation'`, `actor_user_id`/`actor_name` = the verified staffer, `summary` = `"<Action> authorized"`.
- **`app/api/staff/set-pin/route.ts`** — admin/manager only (mirrors `app/api/staff/create`'s auth pattern exactly: bearer token + role check). `POST { userId, pin }` sets/resets a staffer's PIN; `GET` returns a `{ [userId]: boolean }` has-a-PIN map so the Staff page can show status without ever exposing a hash to the client. Surfaced on `/dashboard/staff` as a PIN column (badge + Set/Reset button) next to each staff row, admin/manager only.
- **Rollout**: existing staff have no PIN until an admin sets one via Settings → Staff; selecting them in the confirm dialog shows "No PIN set for this staff member" rather than failing silently.
...
- `database.sql` - Full schema (re-runnable: drops + recreates every table, per its own header comment). Against a **live** org's data, a new feature's tables/RLS/triggers are appended here **and** applied as a standalone additive migration (see Outstanding Manual Steps) instead of re-running the whole destructive file.
  - Tables: `organizations`, `users`, `rooms`, `room_types`, `reservations`, `staff_schedules`, `attendance_logs`, `leave_requests`, `staff_compensation`, `payroll_runs`, `payroll_run_adjustments`, `maintenance_logs`, `audit_logs`, `reservation_charges`, `items`, `reservation_guests`, `payments`, `invoices`, `invoice_counters`, `expenses`, `staff_pins`
...
- `app/api/staff/set-pin/route.ts` - Admin/manager-only: set/reset a staffer's identity-confirmation PIN (POST), has-PIN status map for the Staff page (GET)
- `app/api/confirm-identity/route.ts` - PIN verification + attribution logging for the shared-terminal identity confirmation (Stage 4)
...
- `lib/pin.ts` - `hashPin()`/`verifyPin()` — PIN hashing via Node's built-in `crypto.scrypt` (no dependency added); used by `app/api/confirm-identity` and `app/api/staff/set-pin`
...
**✅ DONE: payroll draft-delete migration.** Added after real usage on `/dashboard/payroll` showed the need — a `payroll_runs` row generated before any attendance is logged (or a duplicate from clicking Generate twice) had no way to be cleaned up, since the original design gave `payroll_runs` no DELETE policy at all (mirroring invoices). Fixed with a **draft-only** DELETE policy (finalized/paid runs stay immutable) plus a Delete button in the UI. `migrations/payroll_run_delete.sql` has been run against the live database (non-destructive — added one policy, replaced one trigger function, no data touched).

**✅ DONE: Accounts / expenses migration.** `migrations/accounts.sql` has been run in the Supabase SQL Editor (created `expenses` + its index + admin/manager-only RLS), and Realtime is enabled for it (Database → Publications → `supabase_realtime` → `expenses` toggled on; the migration already sets `REPLICA IDENTITY FULL`). `/dashboard/accounts` is fully live against the deployed schema.

**⬜ PENDING: `migrations/staff_pins.sql`** — creates the `staff_pins` table (PIN-based identity confirmation, replacing the old password flow; see Shared-Terminal Identity Confirmation above). Non-destructive, no Realtime toggle needed (this table is never subscribed to, same as `invoice_counters`). Run it in the Supabase SQL Editor, then have an admin set a PIN for each active staffer from Settings → Staff before relying on the confirm-identity gate — until a staffer has a PIN, selecting them in that dialog shows "No PIN set."
...
organizations (tenants — incl. currency)
  ...
  └── staff_pins (hashed 4-digit PIN per staffer; no RLS policies — service-role only; shared-terminal identity confirmation)
```
